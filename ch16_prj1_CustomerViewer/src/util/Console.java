...
public class Console {
	...
	public int getIntWithinRange(String prompt, int min, int max) {
		...
		while (!isValid) {
			...
			if (i < min) {
				...
				System.out.println("Error! Highest number must be " + max + ".");
			} else {
				isValid = true;
			}
	...
	public double getDoubleWithinRange(String prompt, double min, double max) {
		...
		while (!isValid) {
			d = getDouble(prompt);
			if (d < min) {
				System.out.println("Error! Double type Number must be greater than " + min + ".");
			} else if (d > max) {
				System.out.println("Error! Double type Number must be less than " + max + ".");
			} else {
				isValid = true;
			}
